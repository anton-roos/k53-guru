...
namespace K53Guru.Server.UI.Hubs;
...
public sealed class HubClient : IAsyncDisposable
{
    ...
    public async Task<List<K53Guru.Application.Common.Interfaces.Identity.UserContext>> GetOnlineUsersAsync(CancellationToken cancellationToken = default)
    {
        var users = await _hubConnection.InvokeAsync<List<K53Guru.Application.Common.Interfaces.Identity.UserContext>>(nameof(ISignalRHub.GetOnlineUsers), cancellationToken).ConfigureAwait(false);
        return users ?? new List<K53Guru.Application.Common.Interfaces.Identity.UserContext>();
    }
}
